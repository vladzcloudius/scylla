...
private:
    ...
    private:
        ...
        enum class state {
            segment_replay_ok,      // current segment sending is still ok
        ...
        using state_set = enum_set<super_enum<state,
            state::segment_replay_ok,
            state::restart_segment,
            state::can_hint,
    ...
    private:
        ...
        /// \brief Try to send the mutation out until it either succeeds or we have to stop trying.
        ///
        ...
        future<> send_one_mutation(mutation m, timer_clock_type::duration timeout);
        ...
        ///
        /// \param sending_began_at time when the timer began sending hints in the current iteration
        ...
        /// \param file_send_gate the gate to use for issueing the write under (we will wait for pending hints to complete using this gate)
        /// \return future that resolves when next hint may be sent
        future<> send_one_hint(timer_clock_type::time_point sending_began_at, temporary_buffer<char> buf, db::replay_position rp, gc_clock::duration secs_since_file_mod, const sstring& fname, seastar::gate& file_send_gate);
        ...
        /// \return future that resolves when sending of the file is complete (succeeded or failed)
        future<> send_one_file(const sstring& fname, timer_clock_type::time_point sending_began_at);
