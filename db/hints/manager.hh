...
private:
    ...
    using hints_segments_map = std::unordered_map<sstring, std::unordered_map<unsigned, std::list<lister::path>>>;
...
private:
    ...
    /// shards (both on the disk and in the \ref segments_map) until we either achieve the requested
    /// \ref segments_per_shard level on each shard or until we are out of files to move.
    ///
    /// As a result (in addition to actual state on the disk) both \ref segments_map and \ref segments_to_move are going
    /// to be modified.
    ///
    ...
    /// \param segments_map a map that was originaly built by get_current_hints_segments()
