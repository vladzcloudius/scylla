...
class entry_writer : public commitlog_file_entry_writer<serialize_one_hint_entry> {
public:


private:
    ts_clock::time_point _ts;
    gc_gs_clock::time_point _min_gc_gs;

public:
    entry_writer(schema_ptr s, const frozen_mutation& fm)
        : commitlog_file_entry_writer(std::move(s), fm)
        , _ts(ts_clock::now())
        , _min_gc_gs(gc_gs_clock::now() + schema()->gc_grace_seconds())
    {}

    virtual ~entry_writer() {}

    ts_clock::time_point ts() const {
        return _ts;
    }

    gc_gs_clock::time_point min_gc_gs() const {
        return _min_gc_gs;
    }
};
...
struct serialize_one_hint_entry {
    ...
    static void do_serialize(Output& out, const commitlog_file_entry_writer<serialize_one_hint_entry>* cl_e_wr) {
        const entry_writer* h_e_wr = static_cast<const entry_writer*>(cl_e_wr);
    ...
    static size_t estimate_size(const commitlog_file_entry_writer<serialize_one_hint_entry>* base_cl_writer) {
