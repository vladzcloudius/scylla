...
future<db::commitlog> manager::end_point_hints_manager::add_store() noexcept {
    ...
    return io_check(recursive_touch_directory, _hints_dir.c_str()).then([this] () {
        ...
        return commitlog::create_commitlog(std::move(cfg), false).then([this] (commitlog l) {
...
future<> manager::end_point_hints_manager::do_send_one_hint(mutation m, const std::vector<gms::inet_address>& natural_endpoints) noexcept {
...
bool manager::end_point_hints_manager::can_send() noexcept {
    try {
        if (!_gossiper.is_alive(end_point_key())) {
            if (_ep_state_is_not_normal) {
                // send the hints out if the destination Node not in a NORMAL state - we will send to all new replicas in this case
                return true;
            } else {
                _ep_state_is_not_normal = _shard_manager._gossiper_anchor->get_gossip_status(end_point_key()) != sstring(gms::versioned_value::STATUS_NORMAL);
            }
            return _ep_state_is_not_normal;
        } else {
            _ep_state_is_not_normal = false;
            return true;
        }
    } catch (...) {
        return false;
...
future<> manager::end_point_hints_manager::send_one_hint(mutation m, timer_clock_type::duration timeout) {
    ...
    return do_with(std::move(m), std::move(natural_endpoints), make_exception_future<>(hint_sending_error()), [this, sending_began_at, timeout] (auto& m, auto& natural_endpoints, future<>& res) {
        return do_until(
            ...
            [this, &m, &natural_endpoints, &res] {
                return do_send_one_hint(m, natural_endpoints).then_wrapped([&res, &m] (auto&& f) {
...
future<> manager::end_point_hints_manager::send_hints() {
    ...
    return do_for_each(_segments_to_replay, [this, sending_began_at] (sstring& fname) {
        return get_last_file_modification(fname).then([this, &fname, sending_began_at] (timespec last_mod) {
            return do_with(seastar::gate(), [this, last_mod, &fname, sending_began_at] (seastar::gate& file_send_gate) {
                _segment_replay_ok = true;
                _rps_set.clear();
                return commitlog::read_log_file(fname, [this, last_mod, &fname, &file_send_gate, sending_began_at] (temporary_buffer<char> buf, db::replay_position rp) {
                    _rps_set.emplace(std::move(rp));
                    ...
                    if (!_segment_replay_ok || !can_send()) {
                    ...
                    return with_semaphore(_shard_manager._send_limiter, hint_memory_budget, [this, last_mod, &fname, &file_send_gate, buf = std::move(buf), sending_began_at, rp] () mutable {
                        with_gate(file_send_gate, [this, last_mod, &fname, &buf, sending_began_at, rp] {
                            try {
                                ...
                                // send the hint
                                return send_one_hint(std::move(m), hints_timer_period - (timer_clock_type::now() - sending_began_at)).then([this, rp] {
                ...
                _last_not_complete_rp = db::replay_position();
