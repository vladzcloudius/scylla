...
future<> manager::stop() {
    ...
    std::for_each(_ep_managers.begin(), _ep_managers.end(), [] (auto& ep_man) {
        ep_man.set_stopping();
    });
...
bool manager::store_hint(ep_key_type ep, schema_ptr s, lw_shared_ptr<const frozen_mutation> fm, tracing::trace_state_ptr tr_state) noexcept {
    ...
    try {
        with_gate(_hints_write_gate, [this, ep, s = std::move(s), fm = std::move(fm), tr_state] () mutable -> future<> {
            return get_ep_manager(ep).store_hint(s, fm).then([this, tr_state, ep] {
...
future<> manager::end_point_hints_manager::do_send_one_mutation(mutation m, const std::vector<gms::inet_address>& natural_endpoints) noexcept {
    ...
    return futurize_apply([this, m = std::move(m), &natural_endpoints] () mutable -> future<> {
        if (boost::range::find(natural_endpoints, end_point_key()) != natural_endpoints.end()) {
            ...
            return _proxy.send_to_endpoint(std::move(m), end_point_key(), write_type::SIMPLE, sp::mutate_flags_set::of<sp::mutate_flags::dont_generate_hints>());
        } else {
            manager_logger.trace("Endpoints set has changed and {} is no longer a replica. Mutating from scratch...", end_point_key());
            return _proxy.mutate({std::move(m)}, consistency_level::LOCAL_QUORUM, nullptr, sp::mutate_flags_set::of<sp::mutate_flags::dont_generate_hints>());
        }
    });
}
...
future<> manager::end_point_hints_manager::send_one_hint(temporary_buffer<char> buf, db::replay_position rp, gc_clock::duration secs_since_file_mod, const sstring& fname, seastar::gate& file_send_gate) {
    ...
    return get_units(_shard_manager._send_limiter, hint_memory_budget).then([this, secs_since_file_mod, &fname, &file_send_gate, buf = std::move(buf), rp] (auto units) mutable {
        with_gate(file_send_gate, [this, secs_since_file_mod, &fname, buf = std::move(buf), rp] () mutable {
            try {
                try {
                    _rps_set.emplace(rp);
                } catch (...) {
    ...
    return get_last_file_modification(fname).then([this, &fname, sending_began_at] (timespec last_mod) {
        ...
        return do_with(seastar::gate(), [this, secs_since_file_mod, &fname, sending_began_at] (seastar::gate& file_send_gate) {
            ...
            return commitlog::read_log_file(fname, [this, secs_since_file_mod, &fname, &file_send_gate, sending_began_at] (temporary_buffer<char> buf, db::replay_position rp) {
                ...
                // Break early if stop() was called or we are sending for too long - we will re-send this segment next time.
                if (!can_send() || timer_clock_type::now() > sending_began_at + hints_timer_period) {
                    _state.remove(state::segment_replay_ok);
                    return make_exception_future(segment_sending_error());
                }

                return send_one_hint(std::move(buf), rp, secs_since_file_mod, fname, file_send_gate);
