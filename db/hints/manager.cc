...
future<> manager::end_point_hints_manager::stop() noexcept {
    auto func = [this] () -> future<> {
        ...
        // First, wait for all storing in progress to complete and shutdown the store backend...
        return _store_gate.close().finally([this] {
            return with_lock(file_update_mutex(), [this] {
        ...
        // ... then shutdown the sending.
        }).finally([this] {
            return _timer_gate.close().finally([this] () { _timer.cancel(); });
        });
    };
...
void manager::end_point_hints_manager::on_timer() {
    ...
    with_gate(_timer_gate, [this, timer_start_tp] {
