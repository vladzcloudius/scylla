            ...
            return with_shared(file_update_mutex(), [this, fm, s, tr_state] () mutable -> future<> {
                return get_or_load().then([this, fm = std::move(fm), s = std::move(s), tr_state] (hints_store_ptr log_ptr) mutable {
                    ...
                    try {
                        ...
                        manager_logger.debug("Got the exception: {}", e.what());
                        tracing::trace(tr_state, "Failed to store a hint to {}: {}", end_point_key(), e.what());
                    } catch (...) {
                        manager_logger.debug("Got the unknown exception");
...
future<> manager::end_point_hints_manager::sender::flush_maybe() {
    ...
    if (current_time >= _next_flush_tp) {
        return _ep_manager.flush_current_hints().then([this, current_time] {
            ...
            manager_logger.trace("flush_maybe(): got the exception");
    ...
    _stopped = seastar::async([this] {
        ...
        while (!_state.contains(state::stopping)) {
            ...
            // log and keep on spinning
                ...
                manager_logger.trace("sender: got the exception: {}", e.what());
            } catch (...) {
                manager_logger.trace("sender: got the unknown exception");
            }
        }
...
void manager::end_point_hints_manager::sender::send_hints_maybe() {
    ...
    // Exceptions are not expected here during the regular operation, so just log them.
        ...
        manager_logger.trace("send_hints(): got the exception: {}", e.what());
    } catch (...) {
        manager_logger.trace("send_hints(): got the unknown exception");
    }

    if (have_segments()) {
        // TODO: come up with something more sphisticated here
