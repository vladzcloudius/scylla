...
bool manager::end_point_hints_manager::store_hint(schema_ptr s, lw_shared_ptr<const frozen_mutation> fm, tracing::trace_state_ptr tr_state) {
    try {
        with_gate(_async_ctx_gate, [this, s = std::move(s), fm = std::move(fm), tr_state] () mutable {
            ++_hints_in_progress;
            ...
            return with_shared(file_update_mutex(), [this, fm, s, tr_state] () mutable -> future<> {
                return get_or_load().then([this, fm = std::move(fm), s = std::move(s), tr_state] (hints_store_ptr log_ptr) mutable {
                    ...
                    try {
                        ...
                        tracing::trace(tr_state, "Failed to store a hint to {}: {}", end_point_key(), e.what());
                    } catch (...) {
                        manager_logger.debug("Got unknown exception");
                        tracing::trace(tr_state, "Failed to store a hint to {}: unknown exception", end_point_key());
                    }
                });
        ...
        manager_logger.trace("Failed to store a hint to {}: store gate is closed", end_point_key());
        tracing::trace(tr_state, "Failed to store a hint to {}: store gate is closed", end_point_key());

        ++shard_stats().dropped;
        return false;
    } catch (...) {
        manager_logger.trace("Failed to store a hint to {}: unexpected exception", end_point_key());
        tracing::trace(tr_state, "Failed to store a hint to {}: unexpected exception", end_point_key());
...
future<> manager::end_point_hints_manager::stop() noexcept {
    auto func = [this] () -> future<> {
        ...
        return _async_ctx_gate.close().finally([this] {
            _timer.cancel();
            return with_lock(file_update_mutex(), [this] {
...
void manager::end_point_hints_manager::on_timer() {
    manager_logger.trace("ep_manager({}): on_timer: start", end_point_key());

    // Drop the columns mappings cache periodically to avoid its bloating.
    auto timer_start_tp = timer_clock_type::now();
    if (timer_start_tp - _last_columns_mapping_cache_flush_tp > _column_mapping_cache_flush_period) {
        _schema_ver_to_column_mapping.clear();
        _last_columns_mapping_cache_flush_tp = timer_start_tp;
    }

    with_gate(_async_ctx_gate, [this, timer_start_tp] {
        return flush_current_hints().then([this] {
            if (can_send()) {
                return send_hints();
            } else {
                manager_logger.trace("ep_manager({}): can't send now", end_point_key());
                return make_ready_future<>();
            }
        }).finally([this, timer_start_tp] {
            manager_logger.trace("ep_manager({}): on_timer: rearming", end_point_key());
...
future<> manager::end_point_hints_manager::send_one_mutation(mutation m) {
...
future<> manager::end_point_hints_manager::send_one_hint(temporary_buffer<char> buf, db::replay_position rp, gc_clock::duration secs_since_file_mod, const sstring& fname) {
    ...
    return get_units(_shard_manager._send_limiter, hint_memory_budget).then([this, secs_since_file_mod, &fname, buf = std::move(buf), rp] (auto units) mutable {
        with_gate(_file_send_gate, [this, secs_since_file_mod, &fname, buf = std::move(buf), rp] () mutable {
            try {
                try {
                    _rps_set.emplace(rp);
                } catch (...) {
                ...
                return send_one_mutation(std::move(m)).then([this, rp] {
                    _rps_set.erase(rp);
                    ++shard_stats().sent;
...
future<> manager::end_point_hints_manager::send_one_file(const sstring& fname, timer_clock_type::time_point sending_began_at) {
    return get_last_file_modification(fname).then([this, &fname, sending_began_at] (timespec last_mod) {
        ...
        _file_send_gate = seastar::gate();
        _state.set(state::segment_replay_ok);
        _rps_set.clear();
        _state.remove(state::restart_segment);
        return commitlog::read_log_file(fname, [this, secs_since_file_mod, &fname, sending_began_at] (temporary_buffer<char> buf, db::replay_position rp) {
            ...
            // Break early if stop() was called, destination node went down or we are sending for too long - we will re-send this segment next time.
            if (!can_send() || timer_clock_type::now() > sending_began_at + hints_timer_period) {
                _state.remove(state::segment_replay_ok);
                return make_exception_future(segment_sending_error());
            }

            return send_one_hint(std::move(buf), rp, secs_since_file_mod, fname);
            ...
            return _file_send_gate.close().finally([this] {
                if (!_state.contains(state::segment_replay_ok)) {
                    ...
                    manager_logger.trace("send_one_file(): Error while sending hints, last RP is {}", _last_not_complete_rp);
                    return make_exception_future(segment_sending_error());
                }
                return make_ready_future<>();
            });
...
future<> manager::end_point_hints_manager::send_hints() {
    ...
    return do_for_each(_segments_to_replay, [this, sending_began_at] (sstring& fname) {
        return send_one_file(fname, sending_began_at).then([this] {
...
mutation manager::end_point_hints_manager::get_mutation(temporary_buffer<char>& buf) {
...
const column_mapping& manager::end_point_hints_manager::get_column_mapping(const frozen_mutation& fm, const hint_entry_reader& hr) {
    ...
    if (cm_it == _schema_ver_to_column_mapping.end()) {
        ...
        cm_it = _schema_ver_to_column_mapping.emplace(fm.schema_version(), *hr.get_column_mapping()).first;
    }
...
bool manager::too_many_in_flight_hints_for(ep_key_type ep) const noexcept {
    ...
    return _stats.size_of_hints_in_progress > _max_size_of_hints_in_progress && !utils::fb_utilities::is_me(ep) && hints_in_progress_for(ep) > 0 && _gossiper_anchor->get_endpoint_downtime(ep) <= _max_hint_window_us;
}
