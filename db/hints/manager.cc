...
future<> manager::end_point_hints_manager::sender::flush_maybe() noexcept {
    auto current_time = sender_clock::now();
    if (current_time - _last_flush_tp > hints_flush_period) {
        return _ep_manager.flush_current_hints().then([this, current_time] {
            _last_flush_tp = current_time;
        });
    }
    return make_ready_future<>();
}
...
void manager::end_point_hints_manager::sender::run() {
    using namespace std::literals::chrono_literals;

    seastar::async([this] {
        manager_logger.trace("ep_manager({})::sender::run(): start", end_point_key());
        while (!_state.contains(state::stopping)) {
            try {
                flush_maybe().get();

                if (_segments_to_replay.size()) {
                    send_hints();
                }
                ...
                // do nothing - keep on spinning
            }
        }
        manager_logger.trace("ep_manager({})::sender::run(): end", end_point_key());
        _stopped.set_value();
    }).discard_result();
}
...
bool manager::end_point_hints_manager::sender::send_one_file(const sstring& fname) {
    ...
    try {
        auto s = commitlog::read_log_file(fname, [this, secs_since_file_mod, &fname, ctx_ptr] (temporary_buffer<char> buf, db::replay_position rp) mutable {
            ...
            return flush_maybe().handle_exception([] (auto eptr) {
                // Ignore flush failures - there is nothing to do about it anyway, keep on sending...
            }).finally([this, ctx_ptr, buf = std::move(buf), rp, secs_since_file_mod, &fname] () mutable {
                return send_one_hint(std::move(ctx_ptr), std::move(buf), rp, secs_since_file_mod, fname);
            });
        }, _last_not_complete_rp.pos).get0();

        // ignore exceptions - we have the status in the ctx_ptr->state
        s->done().get();
    } catch (...) {
        ctx_ptr->state.set(send_state::segment_replay_failed);
    }
...
void manager::end_point_hints_manager::sender::send_hints() noexcept {
    ...
    // keep mappings for a single iteration of a sender
    _schema_ver_to_column_mapping.clear();

    try {
        for(sstring& fname : _segments_to_replay) {
            if (!send_one_file(fname)) {
                break;
            }
            ++replayed_segments_count;
        }
    } catch (...) {
        // Ignore the exceptions, we will re-try to send this file from where we left off the next time.
        // Exceptions are not expected here during the regular operation.
    }

    manager_logger.trace("send_hints(): We handled {} segments", replayed_segments_count);

    if (replayed_segments_count == 0) {
        return;
    }

    auto start = _segments_to_replay.begin();
    auto end = std::next(start, replayed_segments_count);
