...
bool storage_proxy::cannot_hint(gms::inet_address target) {
    // if hints are disabled or target is a local Node - we "can always hint" since there's going to be no hint generated in this case
    return hints_enabled() && !is_me(target) && _local_hints_manager_ptr->too_many_in_flight_hints_for(target);
}
...
{
    if (hints_enabled()) {
        // do not hint to the local address - the mutation is already present in the commitlog
        return boost::count_if(targets | boost::adaptors::filtered([] (auto& target) { return !is_me(target); }),
            [this, &mh, tr_state = std::move(tr_state)](gms::inet_address target) mutable -> bool {
...
void storage_proxy::enable_hints() noexcept {
    _local_hints_manager_ptr = &db::hints::manager::get_local_manager_instance();
}

void storage_proxy::disable_hints() noexcept {
    _local_hints_manager_ptr = nullptr;
}
