...
bool storage_proxy::cannot_hint(gms::inet_address target) {
    ...
    return can_hint(target) && _local_hints_manager_ptr->too_many_in_flight_hints_for(target);
}
...
{
    return boost::count_if(targets | boost::adaptors::filtered(std::bind1st(std::mem_fn(&storage_proxy::can_hint), this)),
...
bool storage_proxy::can_hint(gms::inet_address ep) noexcept {
    return _may_hint && !is_me(ep); // do not hint to local address
}
...
void storage_proxy::allow_hints() noexcept {
    _may_hint = true;
    _local_hints_manager_ptr = &db::hints::manager::get_local_manager_instance();
}

void storage_proxy::forbid_hints() noexcept {
    _may_hint = false;
