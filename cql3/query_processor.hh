...
#include "cql3/authenticated_prepared_statements_cache.hh"
...
private:
    ...
    authenticated_prepared_statements_cache _authenticated_prepared_cache;
...
public:
    ...
    statements::prepared_statement::checked_weak_ptr get_prepared(::shared_ptr<auth::authenticated_user> user, const prepared_cache_key_type& key) {
        if (user) {
            auto it = _authenticated_prepared_cache.find(*user, key);
            if (it != _authenticated_prepared_cache.end()) {
                try {
                    return it->get()->checked_weak_from_this();
                } catch (seastar::default_null_deref_action&) {
                    // If the prepared statement got invalidated - remove the corresponding authenticated_prepared_statements_cache entry as well.
                    _authenticated_prepared_cache.remove(*user, key);
                }
            }
        }
...
private:
    ...
    future<::shared_ptr<cql_transport::messages::result_message>>
    process_authenticated_statement(const ::shared_ptr<cql_statement> statement, service::query_state& query_state, const query_options& options);
