...
struct authorized_prepared_statements_cache_key_type {
public:
...
public:
    authorized_prepared_statements_cache_key_type(auth::authenticated_user user, cql3::prepared_cache_key_type prepared_cache_key)
    ...
    bool operator==(const authorized_prepared_statements_cache_key_type& other) const {
...
private:
    using cache_key_type = authorized_prepared_statements_cache_key_type;
...
namespace std {
template <>
struct hash<cql3::authorized_prepared_statements_cache_key_type> final {
    size_t operator()(const cql3::authorized_prepared_statements_cache_key_type& k) const {
        return utils::hash_combine(std::hash<auth::authenticated_user>()(k.key().first), utils::tuple_hash()(k.key().second));
    }
};

inline std::ostream& operator<<(std::ostream& out, const cql3::authorized_prepared_statements_cache_key_type& k) {
