...
logging::logger user_prepared_auth_cache_log("authorized_prepared_statements_cache");
...
query_processor::query_processor(service::storage_proxy& proxy, distributed<database>& db)
        ...
        , _authorized_prepared_cache(std::chrono::milliseconds(_db.local().get_config().permissions_update_interval_in_ms()), user_prepared_auth_cache_log) {
...
query_processor::process_statement_prepared(
    ...
    if (needs_authorization) {
        fut = statement->check_access(query_state.get_client_state()).then([this, &query_state, prepared = std::move(prepared), cache_key] () mutable {
            return _authorized_prepared_cache.insert(*query_state.get_client_state().user(), cache_key, std::move(prepared)).handle_exception([this] (auto eptr) {
