...
def get_mode_cpuset(nic, mode):
    try:
        mode_cpu_mask = subprocess.check_output(['/usr/lib/scylla/perftune.py', '--tune', 'net', '--nic', nic, '--mode', mode, '--get-cpu-mask'], stderr=subprocess.DEVNULL, env=scylla_env).strip().decode('utf-8')
