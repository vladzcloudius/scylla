...
future<response_type> cql_server::connection::process_execute(uint16_t stream, bytes_view buf, service::client_state client_state)
{
    ...
    // First, try to lookup in the cache of already authenticated statements. If the corresponding entry is not found there
    // look for the prepared statement and then authenticate it.
    ...
    return _server._query_processor.local().process_statement_prepared(std::move(prepared), cache_key, query_state, options, needs_authentication).then([this, stream, buf = std::move(buf), &query_state, skip_metadata] (auto msg) {
        tracing::trace(query_state.get_trace_state(), "Done processing - preparing a result");
        return this->make_result(stream, msg, query_state.get_trace_state(), skip_metadata);
    }).then([&query_state, q_state = std::move(q_state), this] (auto&& response) {
        /* Keep q_state alive. */
