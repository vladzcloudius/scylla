...
SEASTAR_TEST_CASE(test_loading_cache_removing_key) {
    return seastar::async([] {
        using namespace std::chrono;
        load_count = 0;
        ...
        BOOST_REQUIRE_EQUAL(load_count, 1);
        BOOST_REQUIRE_EQUAL(loading_cache.find(0) == loading_cache.end(), false);

        loading_cache.remove(0);
        BOOST_REQUIRE_EQUAL(loading_cache.find(0) == loading_cache.end(), true);
    });
}
